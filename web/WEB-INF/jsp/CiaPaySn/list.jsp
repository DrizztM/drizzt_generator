						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/ciaPaySn/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">MONEY</th>
														<th class="sorting_disabled">DEVELOPER_ID</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${ciaPaySns}" var="ciaPaySn">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${ciaPaySn.id}" class="ace"> <span
															class="lbl"></span>
															</label></td>
															<td>${ciaPaySn.date}</td>
															<td>${ciaPaySn.money}</td>
															<td>${ciaPaySn.developerId}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaPaySn/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaPaySn/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaPaySn/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaPaySn/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
