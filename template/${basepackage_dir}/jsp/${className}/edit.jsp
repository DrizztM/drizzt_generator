			...
			<div class="row">
				<div class="col-xs-12">
					...
					<form id="drizzt-table" class="form-horizontal"
						action="<%=request.getContextPath()%>/${classNameLower}/edit.do"
						method="post">
						<!-- #section:elements.form -->
						<#list table.columns as column>
							<#if column.htmlHidden>
								<input type="hidden" id="form-field-1" name="${column.columnNameLower}" value="<@jspEl classNameLower+'.'+column.columnNameLower />"
									class="col-xs-10 col-sm-5" />
							<#elseif !column.isDateTimeColumn>
								<div class="form-group">
									<label class="col-sm-3 control-label no-padding-right"
										for="form-field-1"> ${column.columnNameLower} </label>
		
									<div class="col-sm-9">
										<input type="text" id="form-field-1" placeholder="${column.columnNameLower}"
											name="${column.columnNameLower}" value="<@jspEl classNameLower+'.'+column.columnNameLower />"
											class="col-xs-10 col-sm-5" />
									</div>
								</div>
							</#if>
