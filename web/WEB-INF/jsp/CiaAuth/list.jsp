										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">TRANS_ID</th>
														<th class="sorting_disabled">IMSI</th>
														<th class="sorting_disabled">APP_ID</th>
														<th class="sorting_disabled">AUTH_KEY</th>
														<th class="sorting_disabled">PN</th>
														<th class="sorting_disabled">AUTH_CODE</th>
														<th class="sorting_disabled">DURATION</th>
														<th class="sorting_disabled">DATE</th>
														<th class="sorting_disabled">STATUS</th>
														<th class="sorting_disabled">REASON</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${ciaAuths}" var="ciaAuth">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${ciaAuth.id}" class="ace"> <span
															class="lbl"></span>
															</label></td>
															<td>${ciaAuth.transId}</td>
															<td>${ciaAuth.imsi}</td>
															<td>${ciaAuth.appId}</td>
															<td>${ciaAuth.authKey}</td>
															<td>${ciaAuth.pn}</td>
															<td>${ciaAuth.authCode}</td>
															<td>${ciaAuth.duration}</td>
															<td>${ciaAuth.date}</td>
															<td>${ciaAuth.status}</td>
															<td>${ciaAuth.reason}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuth/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuth/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuth/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuth/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
