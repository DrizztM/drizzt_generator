						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/ciaSuggest/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">ISSUE_DATE</th>
														<th class="sorting_disabled">REPLY_DATE</th>
												</tr>
											</thead>
											...
											<tbody>
												<c:forEach items="${ciaSuggests}" var="ciaSuggest">
													<tr role="row" class="odd">
															...
															<input id="id" name="id" type="radio" value="${ciaSuggest.id}" class="ace"> <span
															class="lbl"></span>
															</label></td>
															<td>${ciaSuggest.title}</td>
															<td>${ciaSuggest.content}</td>
															<td>${ciaSuggest.reply}</td>
															<td>${ciaSuggest.issueDate}</td>
															<td>${ciaSuggest.replyDate}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaSuggest/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaSuggest/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaSuggest/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaSuggest/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
