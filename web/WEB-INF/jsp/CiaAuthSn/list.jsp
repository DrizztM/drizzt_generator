						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/ciaAuthSn/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">TIMESTAMP</th>
														<th class="sorting_disabled">STEP</th>
														<th class="sorting_disabled">AUTH_ID</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${ciaAuthSns}" var="ciaAuthSn">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${ciaAuthSn.id}" class="ace"> <span
															class="lbl"></span>
															</label></td>
															<td>${ciaAuthSn.timestamp}</td>
															<td>${ciaAuthSn.step}</td>
															<td>${ciaAuthSn.authId}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuthSn/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuthSn/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuthSn/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuthSn/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
