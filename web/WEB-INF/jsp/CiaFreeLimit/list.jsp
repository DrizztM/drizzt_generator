						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/ciaFreeLimit/list.do"
										...
										<table
											...
											<thead>
												<tr role="row">
														...
														<th class="sorting_disabled">NUM</th>
												</tr>
											</thead>

											<tbody>
												<c:forEach items="${ciaFreeLimits}" var="ciaFreeLimit">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${ciaFreeLimit.id}" class="ace"> <span
															class="lbl"></span>
															</label></td>
															<td>${ciaFreeLimit.appId}</td>
															<td>${ciaFreeLimit.pn}</td>
															<td>${ciaFreeLimit.num}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaFreeLimit/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaFreeLimit/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaFreeLimit/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaFreeLimit/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
