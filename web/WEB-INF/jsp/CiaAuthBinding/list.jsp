						...
						<div class="col-xs-12">
							...
							<div>
								<div id="sample-table-2_wrapper"
									...
									<form action="<%=request.getContextPath()%>/ciaAuthBinding/list.do"
										...
										<table
											...
											<tbody>
												<c:forEach items="${ciaAuthBindings}" var="ciaAuthBinding">
													<tr role="row" class="odd">
															<td class="center"><label class="position-relative">
															<input id="id" name="id" type="radio" value="${ciaAuthBinding.id}" class="ace"> <span
															class="lbl"></span>
															</label></td>
															<td>${ciaAuthBinding.pn}</td>
															<td>${ciaAuthBinding.imsi}</td>
													</tr>
												</c:forEach>
											</tbody>
										</table>
...
<script type="text/javascript">
	...
	function goOperate(operate){
		if(operate=="add"){
			$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuthBinding/toAdd.do");
			$("#drizzt-table").submit();
		}
		if(operate=="edit"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuthBinding/toEdit.do");
				$("#drizzt-table").submit();
            }
		}
		if(operate=="del"){
			...
			if(val==null){
            	...
            	if(confirm("确定要删除该信息吗？删除将不能恢复！")){
            		$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuthBinding/del.do");
     				$("#drizzt-table").submit();
            	}else{
            		return false;
            	}
		...
		if(operate=="view"){
			...
			if(val==null){
				...
				$("#drizzt-table").attr("action", "<%=request.getContextPath()%>/ciaAuthBinding/view.do");
				$("#drizzt-table").submit();
            }
		}
	}
